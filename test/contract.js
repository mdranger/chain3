...
var StandAloneContract = require('../packages/chain3-mc-contract');
...
var getEthContractInstance = function(abi, address, options, provider) {
    ...
    return new mc.Contract(abi, address, options);
}
...
var runTests = function(contractFactory) {
    ...
    describe('internal method', function () {
        ...
        it('_executeMethod should sendTransaction and check for receipts', function (done) {
            ...
            provider.injectValidation(function (payload) {
                assert.equal(payload.method, 'mc_subscribe');
            ...
            provider.injectValidation(function (payload) {
                assert.equal(payload.method, 'mc_unsubscribe');
    ...
    describe('event', function () {
        ...
        it('should create event from the events object and use the fromBlock option', function (done) {
            ...
            provider.injectValidation(function (payload) {
                assert.equal(payload.jsonrpc, '2.0');
                //assert.equal(payload.method, 'mc_getLogs');
            });
            provider.injectResult([{
                    address: addressLowercase,
                    topics: [
    ...
    describe('with methods', function () {
        ...
        it('should estimate a function', function (done) {
            ...
            provider.injectValidation(function (payload) {
                assert.equal(payload.method, 'mc_estimateGas');
                assert.deepEqual(payload.params, [{
                    data: sha3(signature).slice(0, 10) + '000000000000000000000000'+ addressLowercase.replace('0x',''),
                    to: addressLowercase
                }]);
    ...
    describe('with data', function () {
        ...
        it('should deploy a contract and use all promise steps', function (done) {
            ...
            provider.injectValidation(function (payload) {
                assert.equal(payload.method, 'mc_getCode');
...
describe('typical usage', function() {
    ...
    it('should update contract instance provider when assigned a provider to mc instance that contract instance came from', function () {
        ...
        var mc = new Mc(provider1);
        var contract = new mc.Contract(abi, address);
        assert.deepEqual(contract.currentProvider, provider1);
        assert.deepEqual(mc.currentProvider, provider1);

        mc.setProvider(provider2);
    ...
    it('should deploy a contract, sign transaction, and return contract instance', function (done) {
        var provider = new FakeIpcProvider();
        var mc = new Mc(provider);
        mc.accounts.wallet.add(account.privateKey);

        provider.injectValidation(function (payload) {
            var expected = mc.accounts.wallet[0].signTransaction({
                ...
                assert.equal(payload.method, 'mc_sendRawTransaction');
        ...
        var contract = new mc.Contract(abi);
